...
class MultimodalModelRunner:
    ...
    def __init__(self, args):
        ...
        if self.model_type == "qwen2_5_vl":
            ...
            self.image_token_id = hf_config.image_token_id
            self.video_token_id = hf_config.video_token_id
            self.tokens_per_second = hf_config.vision_config.tokens_per_second
            self.spatial_merge_size = hf_config.vision_config.spatial_merge_size
            self.max_position_embeddings = hf_config.max_position_embeddings
            self.hidden_size = hf_config.hidden_size 
            self.num_attention_heads = hf_config.num_attention_heads
    ...
    def init_image_encoder(self):
        ...
        self.visual_encoder_session_0 = Session.from_serialized_engine(
            engine_buffer)
        ...
        with open(vision_encoder_path, 'rb') as f:
            engine_buffer = f.read()
        logger.info(f'Creating session from engine {vision_encoder_path}')
        self.visual_encoder_session_1 = Session.from_serialized_engine(
            engine_buffer)
        print('\n initialized visual encoder')
        ...
        if self.decoder_llm:
            ...
            if self.use_py_session:
                ...
                self.model = ModelRunnerCpp.from_dir(
                    self.args.llm_engine_dir,
                    # './tmp/trt_engines/Qwen2-VL/fp16/1-gpu/llm',
                    rank=tensorrt_llm.mpi_rank(),
                    debug_mode=False,
    ...
    def preprocess(self, warmup, pre_prompt, post_prompt, image,
        ...
        if self.model_type == 'kosmos-2':
            ...
            input_ids = input['input_ids']
            other_vision_inputs['image_grid_thw'].shape[0]
            full_attention_mask = other_vision_inputs['full_attention_mask_llm']
            other_vision_inputs.pop('full_attention_mask_llm')
            window_attention_mask = other_vision_inputs['window_attention_mask_llm']
            other_vision_inputs.pop('window_attention_mask_llm')
            image_grid_thw = other_vision_inputs['image_grid_thw']
            other_vision_inputs.pop('image_grid_thw')
        ...
        if self.model_type == 'fuyu':
            ...
            mid_compute_start = time.time()
            length = input_ids.shape[1]
            input_lengths = torch.IntTensor([length] * self.args.batch_size).to(
                torch.int32)
            input_ids, ptuning_args, mrope_args = self.setup_fake_prompts_qwen2_5_vl(
                visual_features, input_ids, image_grid_thw, full_attention_mask, window_attention_mask,
                input_lengths)
            self.mid_compute_time = time.time() - mid_compute_start
            return input_ids, input_lengths, ptuning_args, visual_features, mrope_args
    ...
    def generate(self,
        ...
        if self.decoder_llm and self.model_type != "mllama":
            ...
            with nvtx.annotate('LLM', color='red'):
                output_ids = self.model.generate(
                    ...
                    return_dict=False)
            print(f'Language model duration {time.time() - l_start} s')
        ...
        if mpi_rank() == 0:
            ...
            print(f'New tokens {output_ids[0, :, input_lengths[0]:][0][output_ids[0, :, input_lengths[0]:][0] != 151645].shape[0]}')
            print(f'E2E duration {self.vision_time + self.mid_compute_time + time.time() - l_start} s')
    ...
    def get_visual_features(self, image, other_vision_inputs):
        if self.model_type == "qwen2_vl":
            ...
            other_vision_inputs['full_attention_mask'] = other_vision_inputs[
                'full_attention_mask'].to(str_dtype_to_torch(self.vision_precision))
            other_vision_inputs['window_attention_mask'] = other_vision_inputs[
                'window_attention_mask'].to(str_dtype_to_torch(self.vision_precision))
            other_vision_inputs['rotary_pos_emb'] = other_vision_inputs[
                'rotary_pos_emb'].to(str_dtype_to_torch(self.vision_precision))
            window_index = other_vision_inputs['window_index']
            other_vision_inputs.pop('window_index')
            # pass
        ...
        visual_output_info = self.visual_encoder_session_0.infer_shapes(tensor_info)
        self.visual_encoder_session_0.set_shapes(visual_features)
        visual_outputs = {
            t.name:
        ...
        import time
        start_1 = time.time()
        with nvtx.annotate('Vision Encoder 0', color='blue'):
            ok = self.visual_encoder_session_0.run(visual_features, visual_outputs,
                                                self.stream.cuda_stream)
            assert ok, "Runtime execution failed for vision encoder session"
            self.stream.synchronize()
        visual_sum = time.time() - start_1
        
        vision_patch_embeds = visual_outputs[self.vision_output_names[0]]
        
        ## data intra-process
        hidden_states = vision_patch_embeds
        ...
        rotary_pos_emb = other_vision_inputs['rotary_pos_emb']
        ...
        visual_features = {
            self.vision_input_names[0]:
            hidden_states.to(str_dtype_to_torch(self.vision_precision)),
        }
        for key, tensor in other_vision_inputs.items():
            visual_features.update({key: tensor})
        tensor_info = [
            TensorInfo(self.vision_input_names[0],
                       str_dtype_to_trt(self.vision_precision), hidden_states.shape),
        ]
        for key, tensor in other_vision_inputs.items():
            tensor_info.append(
                TensorInfo(key, torch_dtype_to_trt(tensor.dtype), tensor.shape))
        visual_output_info = self.visual_encoder_session_1.infer_shapes(
            tensor_info)
        self.visual_encoder_session_1.set_shapes(visual_features)
        visual_outputs = {
            t.name:
            torch.empty(tuple(t.shape),
                        dtype=trt_dtype_to_torch(t.dtype),
        ...
        start_1 = time.time()
        with nvtx.annotate('Vision Encoder 1', color='yellow'):
            ok = self.visual_encoder_session_1.run(visual_features, visual_outputs,
                                                self.stream.cuda_stream)
            assert ok, "Runtime execution failed for vision encoder session"
            self.stream.synchronize()
        ...
        self.vision_time = time.time() - start_1 + visual_sum
        print(f'Visual encoder duration {self.vision_time} s')

        # image_embeds = torch.load('image_embeds.pt').to(image_embeds.device).to(image_embeds.dtype)
    ...
    def setup_fake_prompts_qwen2_5_vl(self, visual_features, input_ids,
                                        vision_grid_thws, full_attention_mask, window_attention_mask,
                                        input_lengths):

        visual_features = torch.unsqueeze(visual_features, 0)

        #generate mrope_params
        mrope_position_ids, mrope_position_deltas = self.get_rope_index_qwen2_5_vl(
            input_ids,
            image_grid_thw=vision_grid_thws,
            video_grid_thw=None,
    ...
    def setup_inputs(self, input_text, raw_image):
        from my_multimodal_builder import compute_rotary_pos_emb, compute_window_index
        other_vision_inputs = {}
        other_decoder_inputs = {}
        if 'qwen2_vl' not in self.model_type and 'qwen2_5_vl' not in self.model_type:
        ...
        if 'blip2' in self.model_type:
            ...
            hf_config = AutoConfig.from_pretrained(self.args.hf_model_dir)
            if input_text is None:
                input_text = ["描述图片。"] * self.args.batch_size
            messages = [[{
                "role":
                "user",
                "content": [
            ...
            hf_config = AutoConfig.from_pretrained(self.args.hf_model_dir)
            if input_text is None:
                 input_text = ["你是一个智能助手，我是用户，请识别这张图片并描述这张图片展示了什么内容。"] * self.args.batch_size
            messages = [[{
                "role":
                "user",
                "content": [
            ...
            inputs = inputs.to(self.device)
            image = inputs['pixel_values'].to(torch.float32)
            image_grid_thw = inputs['image_grid_thw']
            input_ids = inputs['input_ids']
            full_attention_mask_llm = inputs['attention_mask']
            window_attention_mask_llm = inputs['attention_mask']
            cu_seqlens = torch.repeat_interleave(
                image_grid_thw[:, 1] * image_grid_thw[:, 2],
            ...
            full_attention_mask_vit = generate_attnmask(seq_length, cu_seqlens)
            window_attention_mask_vit = generate_attnmask(seq_length, cu_window_seqlens)
            ...
            rotary_pos_emb = compute_rotary_pos_emb(
                image_grid_thw, hf_config, Qwen2_5_VisionRotaryEmbedding).to("cuda").to(image.dtype)
            other_vision_inputs['full_attention_mask_llm'] = full_attention_mask_llm
            other_vision_inputs['window_attention_mask_llm'] = window_attention_mask_llm
            other_vision_inputs['image_grid_thw'] = image_grid_thw
            other_vision_inputs['full_attention_mask'] = full_attention_mask_vit     # to vision
            other_vision_inputs['window_attention_mask'] = window_attention_mask_vit
            other_vision_inputs['rotary_pos_emb'] = rotary_pos_emb
            window_index = window_index.to(torch.int32)
            other_vision_inputs['window_index'] = window_index
