...
def build_trt_engine(model_type,
    ...
    if model_type == "qwen2_vl":
        ...
        if 'model_0' in onnx_name:
            ...
            image.shape = [-1, qwen2_vl_dim]
            profile.set_shape(image.name, [multi_size_min, qwen2_vl_dim],
                            [multi_size_opt, qwen2_vl_dim],
                            [multi_size_max, qwen2_vl_dim])
            
        elif 'model_1' in onnx_name:
            hidden_states = network.get_input(0)
            rotary_pos_emb = network.get_input(1)
            full_attention_mask = network.get_input(2)
            window_attention_mask = network.get_input(3)
            ...
            hidden_states.shape = [-1, qwen2_vl_dim]
            profile.set_shape(hidden_states.name, [multi_size_min, qwen2_vl_dim],
                            [multi_size_opt, qwen2_vl_dim],
                            [multi_size_max, qwen2_vl_dim])

            rotary_pos_emb.shape = [-1, *input_sizes]
            profile.set_shape(rotary_pos_emb.name, [multi_size_min, *input_sizes],
                            [multi_size_opt, *input_sizes],
                            [multi_size_max, *input_sizes])


            full_attention_mask.shape = [1, -1, -1]
            profile.set_shape(full_attention_mask.name,
                            [1, multi_size_min, multi_size_min],
                            [1, multi_size_opt, multi_size_opt],
                            [1, multi_size_max, multi_size_max])
            
            window_attention_mask.shape = [1, -1, -1]
            profile.set_shape(window_attention_mask.name,
                            [1, multi_size_min, multi_size_min],
                            [1, multi_size_opt, multi_size_opt],
                            [1, multi_size_max, multi_size_max])
...
def compute_rotary_pos_emb(grid_thw, hf_config, VisionRotaryEmbedding):
    if hf_config.architectures[0] == 'Qwen2_5_VLForConditionalGeneration':
        ...
        "content": [
            {
                ...
                "image",
                "image":
                "/sys/fs/cgroup/zzz_tmp/Qwen2.5-VL-3B-Instruct.axera/assets/demo.jpg",
            },
            {
                "type": "text",
...
def compute_window_index(hf_config, grid_thw):
    window_index: list = []
    cu_window_seqlens: list = [0]
    window_index_id = 0
    vit_merger_window_size = hf_config.vision_config.window_size // hf_config.vision_config.spatial_merge_size // hf_config.vision_config.patch_size
    spatial_merge_unit = hf_config.vision_config.spatial_merge_size * hf_config.vision_config.spatial_merge_size
    
    for grid_t, grid_h, grid_w in grid_thw:
        llm_grid_h, llm_grid_w = (
            grid_h // hf_config.vision_config.spatial_merge_size,
            grid_w // hf_config.vision_config.spatial_merge_size,
        )
...
def build_qwen2_5_vl_engine(args):
    ...
    from transformers.models.qwen2_5_vl.modeling_qwen2_5_vl import (
        ...
        Qwen2_5_VLVisionAttention, Qwen2_5_VLVisionSdpaAttention, Qwen2_5_VisionRotaryEmbedding, Qwen2_5_VisionPatchEmbed)
    ...
    hf_config = AutoConfig.from_pretrained(args.model_path)
    qwen_2_5_vl_dim = hf_config.vision_config.in_chans * hf_config.vision_config.patch_size * hf_config.vision_config.patch_size * hf_config.vision_config.temporal_patch_size
    processor = AutoProcessor.from_pretrained(args.model_path)
    messages = [{
        "role":
        "user",
        "content": [
            {
                ...
                "image",
                "image":
                "/sys/fs/cgroup/zzz_tmp/Qwen2.5-VL-3B-Instruct.axera/assets/demo1.jpg",
            },
            {
                "type": "text",
                "text": "Describe this picture?"
    ...
    image = inputs['pixel_values'].to(torch.bfloat16)
    image_grid_thw = inputs['image_grid_thw']
    cu_seqlens = torch.repeat_interleave(
        image_grid_thw[:, 1] * image_grid_thw[:, 2],
        image_grid_thw[:, 0]).cumsum(dim=0, dtype=image_grid_thw.dtype if torch.jit.is_tracing() else torch.int32)
    cu_seqlens = F.pad(cu_seqlens, (1, 0), value=0)
    
    # prepare window_index & cu_window_seqlens
    window_index, cu_window_seqlens = compute_window_index(hf_config, image_grid_thw)
    ...
    full_attention_mask = generate_attnmask(seq_length, cu_seqlens)
    window_attention_mask = generate_attnmask(seq_length, cu_window_seqlens)
    
    # prepare rotary_pos_emb
    rotary_pos_emb = compute_rotary_pos_emb(image_grid_thw, hf_config,
    ...
    def scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None) -> torch.Tensor:
        ...
        scale_factor = 1 / torch.sqrt(query.size(-1)) if scale is None else scale
    ...
    class Qwen2_5_VLVisionSdpaAttentionOpt(Qwen2_5_VLVisionSdpaAttention):
    ...
    class Qwen2_5_VLVisionAttentionOpt(Qwen2_5_VLVisionAttention):
    ...
    class Qwen2_5_VLVisionBlockOpt(Qwen2_5_VLVisionBlock):
        ...
        def __init__(self, config, attn_implementation: str = "sdpa"):
            super().__init__(config)
            # implemented by TRT with flash attn 
            self.attn = Qwen2_5_VLVisionAttentionOpt(config.hidden_size,
                                           config.num_heads)
            # self.attn = Qwen2_5_VLVisionSdpaAttentionOpt(
        ...
        def forward(self,
            ...
            hidden_states = hidden_states + self.attn(
                self.norm1(hidden_states), attention_mask, rotary_pos_emb)
            hidden_states = hidden_states + self.mlp(self.norm2(hidden_states))
            return hidden_states
    
    class Qwen2_5_VisionTransformerPretrainedModelOpt(Qwen2_5_VisionTransformerPretrainedModel):
        def __init__(self, config) -> None:
            super().__init__(config)
            self.blocks = nn.ModuleList(
                [Qwen2_5_VLVisionBlockOpt(config) for _ in range(config.depth)]
            )
        ...
        def forward(self, hidden_states: torch.Tensor,
            ...
            # grid_thw = torch.tensor([[1, 36, 36]])
            # window_index, _ = self.get_window_index(grid_thw)
            ...
            for layer_num, blk in enumerate(self.blocks):
                ...
                hidden_states = blk(hidden_states, attention_mask, rotary_pos_emb)
    ...
    class VisionEncoderWrapper(torch.nn.Module):
        def __init__(self, model, dtype):
            super().__init__()
            self.visual = Qwen2_5_VisionTransformerPretrainedModelOpt._from_config(
                model.config.vision_config,
                torch_dtype=dtype,
            )
            self.visual.load_state_dict(model.visual.state_dict())
            
        def forward(self, images, rotary_pos_emb, full_attention_mask, window_attention_mask,
                    # img_hw,
                    # window_index
                    ):
            
            img_features = self.visual(images, rotary_pos_emb, full_attention_mask, window_attention_mask,
                                    #    img_hw,
                                    #    window_index
                                       )
            return img_features
        
    # class Qwen2_5_VisionPatchEmbedOpt(Qwen2_5_VisionPatchEmbed):
    
    wrapper = VisionEncoderWrapper(model, torch.bfloat16)
    
    # export model_0
    vision_patch_embed = wrapper.visual.patch_embed
    dynamic_axes = {
        'input': {
            0: 'hw'
        },
    ...
    # import pdb;pdb.set_trace()
    print('\n\n Exporting Model_0 ONNX ...\n\n')
    export_onnx(vision_patch_embed, (image),
                f'{args.output_dir}/onnx',
                onnx_name='model_0.onnx',
                input_names=['input'],
                output_names=['output'],
                dynamic_axes=dynamic_axes,
                )
    ...
    print('\n\n Exporting Model_0 TRT ...\n\n')
    build_trt_engine(args.model_type, [rotary_pos_emb_dim],
                     f'{args.output_dir}/onnx',
                     args.output_dir,
                     args.max_batch_size,
                     onnx_name='model_0.onnx',
                     engine_name='model_0.engine',
                     dtype=torch.bfloat16,
                     qwen2_vl_dim=qwen_2_5_vl_dim,
                     min_hw_dims=args.min_hw_dims,
                     max_hw_dims=args.max_hw_dims)
    ...
    hidden_states = vision_patch_embed(image)
    seq_len, _ = hidden_states.size()
    hidden_states = hidden_states.reshape(seq_len // wrapper.visual.spatial_merge_unit, wrapper.visual.spatial_merge_unit, -1)
    hidden_states = hidden_states[window_index, :, :]
    hidden_states = hidden_states.reshape(seq_len, -1)
    rotary_pos_emb = rotary_pos_emb.reshape(seq_len // wrapper.visual.spatial_merge_unit, wrapper.visual.spatial_merge_unit, -1)
    rotary_pos_emb = rotary_pos_emb[window_index, :, :]
    rotary_pos_emb = rotary_pos_emb.reshape(seq_len, -1)
    # import pdb;pdb.set_trace()
    wrapper = VisionEncoderWrapper(model, torch.float32)
    window_index = window_index.to(torch.int32)
    rotary_pos_emb = rotary_pos_emb.to(image.dtype)
    print('\n\n Exporting Model_1 ONNX ...\n\n')
    export_onnx(wrapper, (hidden_states, rotary_pos_emb, full_attention_mask, window_attention_mask),
                f'{args.output_dir}/onnx',
                onnx_name='model_1.onnx',
                input_names=['input', 'rotary_pos_emb', 'full_attention_mask', 'window_attention_mask'],
                output_names=['output'],
                dynamic_axes=dynamic_axes,
                )
    rotary_pos_emb_dim = hf_config.vision_config.hidden_size // hf_config.vision_config.num_heads // 2
    print('\n\n Exporting Model_1  TRT ...\n\n')
    build_trt_engine(args.model_type, [rotary_pos_emb_dim],
                     f'{args.output_dir}/onnx',
                     args.output_dir,
                     args.max_batch_size,
                     onnx_name='model_1.onnx',
                     engine_name='model_1.engine',
                     dtype=torch.bfloat16,
                     qwen2_vl_dim=hf_config.vision_config.hidden_size,
                     min_hw_dims=args.min_hw_dims,
                     max_hw_dims=args.max_hw_dims)
