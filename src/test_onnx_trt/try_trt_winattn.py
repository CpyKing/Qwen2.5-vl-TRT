...
class MyModel(nn.Module):
    ...
    def forward_navie(self,
        ...
        # attn_weight = torch.full((self.num_heads, seq_length, seq_length), torch.finfo(q.dtype).min, device=q.device)
        for i in range(1, len(self.cu_window_seqlens)):
            start = self.cu_window_seqlens[i - 1]
            end = self.cu_window_seqlens[i]
            attn_weight[:, start:end, start:end] = torch.matmul(q[:, start:end, :], k[:, start:end, :].transpose(-1, -2))
        attn_weight = attn_weight / torch.sqrt(torch.tensor(self.head_dim).to(q.device))
...
attn_weight = torch.full((16, hidden_states.shape[0], hidden_states.shape[0]), torch.finfo(hidden_states.dtype).min, device=hidden_states.device)
# model(hidden_states, attn_weight, rotary_pos_emb)

# export ONNX
# torch.onnx.export(
#     model,
#     (hidden_states, attn_weight, rotary_pos_emb),
#     "custom_model.onnx",
#     input_names=['input', 'attn_weight', 'rotary_pos_emb'],
#     output_names=['output'],
#     opset_version=17,
# )
...
# print('input is \n', dummy_input)
## run torch
# print('torch output is \n', torch_forward(model, hidden_states, attn_weight, rotary_pos_emb))
...
print('trt qwen output is \n', trt_forward([hidden_states, attn_weight, rotary_pos_emb], 'float16'))
