...
class MultimodalModelRunner:
    ...
    def get_visual_features(self, image, other_vision_inputs):
        ...
        print(f'Current duration: {time.time()-start:.6f} s\n')
    ...
    def setup_inputs(self, input_text, raw_image):
        ...
        if 'blip2' in self.model_type:
            ...
            # full_attention_mask_vit = torch.full([1, seq_length, seq_length],
            #                                 torch.finfo(torch.float16).min,
            #                                 device=image.device,
            #                                 dtype=image.dtype)
            # window_attention_mask_vit = torch.full([1, seq_length, seq_length],
            #                                 torch.finfo(torch.float16).min,
            #                                 device=image.device,
            #                                 dtype=image.dtype)
            
            # for i in range(1, len(cu_seqlens)):
            #     full_attention_mask_vit[..., cu_seqlens[i - 1]:cu_seqlens[i],
            #                 cu_seqlens[i - 1]:cu_seqlens[i]] = 0
            # for i in range(1, len(cu_window_seqlens)):
            #     window_attention_mask_vit[..., cu_window_seqlens[i - 1]:cu_window_seqlens[i],
