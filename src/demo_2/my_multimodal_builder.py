...
def build_trt_engine(model_type,
    ...
    if model_type == "qwen2_vl":
        ...
        # window_index = network.get_input(4)
        ...
        # window_index.shape = [-1]
        # profile.set_shape(window_index.name, [multi_size_min], [multi_size_opt], [multi_size_max])
...
def build_qwen2_5_vl_engine(args):
    ...
    from transformers.models.qwen2_5_vl.modeling_qwen2_5_vl import (
        ...
        Qwen2_5_VLVisionAttention, Qwen2_5_VLVisionSdpaAttention, Qwen2_5_VisionRotaryEmbedding, Qwen2_5_VLVisionFlashAttention2)
        ...
        "content": [
            {
                ...
                "image",
                "image":
                "/sys/fs/cgroup/zzz_tmp/my_dataset/images/womandog.jpeg",
            },
            {
                "type": "text",
                "text": "Describe this picture?"
    ...
    torch.save(window_index, 'window_index.pt')
    torch.save(cu_seqlens, 'cu_seqlens.pt')
    torch.save(cu_window_seqlens, 'cu_window_seqlens.pt')
    ...
    import pdb;pdb.set_trace()
    # full_attention_mask = torch.full([1, seq_length, seq_length],
    #                             torch.finfo(image.dtype).min,
    #                             device=image.device,
    #                             dtype=image.dtype)
    # window_attention_mask = torch.full([1, seq_length, seq_length],
    #                             torch.finfo(image.dtype).min,
    #                             device=image.device,
    #                             dtype=image.dtype)
    # for i in range(1, len(cu_seqlens)):
    #     full_attention_mask[..., cu_seqlens[i - 1]:cu_seqlens[i],
    #                         cu_seqlens[i - 1]:cu_seqlens[i]] = 0
    # for i in range(1, len(cu_window_seqlens)):
    #     window_attention_mask[..., cu_window_seqlens[i - 1]:cu_window_seqlens[i],
    #                             cu_window_seqlens[i - 1]:cu_window_seqlens[i]] = 0
    ...
    class Qwen2_5_VLVisionAttentionOpt(Qwen2_5_VLVisionAttention):
        ...
        def forward(self,
            ...
            k = k.transpose(0, 1)
            v = v.transpose(0, 1)
            attn_weight = torch.matmul(q, k.transpose(-1, -2)) / torch.sqrt(self.head_dim)
    ...
    class Qwen2_5_VisionTransformerPretrainedModelOpt(Qwen2_5_VisionTransformerPretrainedModel):
        def __init__(self, config) -> None:
            ...
            # self.window_index, _ = self.get_window_index(grid_thw)
            self.window_index = torch.load('window_index.pt')
        ...
        def forward(self, hidden_states: torch.Tensor,
            ...
            self.window_index = self.window_index.to(hidden_states.device)
    ...
    class VisionEncoderWrapper(torch.nn.Module):
        ...
        def forward(self, images, rotary_pos_emb, full_attention_mask, window_attention_mask,
            ...
            # import pdb;pdb.set_trace()
            # window_index = window_index[:window_index.shape[0] // 4]
    ...
    import pdb;pdb.set_trace()
    rotary_pos_emb = rotary_pos_emb.to(image.dtype).to(image.device)
    full_attention_mask = full_attention_mask.to(image.device)
    window_attention_mask = window_attention_mask.to(image.device)
    export_onnx(wrapper, (image, rotary_pos_emb, full_attention_mask, window_attention_mask),
                f'{args.output_dir}/onnx',
